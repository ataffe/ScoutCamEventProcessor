...
def sample_rules():
    return [
        RuleEvaluationInput(rule_public_id='1', rule_name='Person Detection', rule='a person is present'),
        RuleEvaluationInput(rule_public_id='2', rule_name='Vehicle Detection', rule='a vehicle is present'),
    ]
...
def initialized_model():
    instance = Gemma4RulesModel(model_variant='test-variant', model_weights_dir='test_weights')
...
def test_constructor_sets_variant_and_weights_dir():
    model = Gemma4RulesModel(model_variant='gemma4-e2b-it', model_weights_dir='ml_weights')
...
def test_init_skips_download_when_weights_are_present(tmp_path):
    ...
    model = Gemma4RulesModel(model_variant='test-variant', model_weights_dir=str(tmp_path))
    with patch('src.ml.gemma4_rules_model.AutoModelForCausalLM'), \
         patch('src.ml.gemma4_rules_model.AutoProcessor'), \
         patch('src.ml.gemma4_rules_model.download_weights_s3') as mock_download:
        model.init()
...
def test_init_downloads_weights_when_dir_does_not_exist(tmp_path):
    ...
    model = Gemma4RulesModel(model_variant='test-variant', model_weights_dir=weights_dir)
    with patch('src.ml.gemma4_rules_model.AutoModelForCausalLM'), \
         patch('src.ml.gemma4_rules_model.AutoProcessor'), \
         patch('src.ml.gemma4_rules_model.download_weights_s3') as mock_download:
        model.init()
    mock_download.assert_called_once_with('gemma4', 'test-variant', weights_dir)
...
def test_init_downloads_weights_when_dir_is_empty(tmp_path):
    ...
    with patch('src.ml.gemma4_rules_model.AutoModelForCausalLM'), \
         ...
         patch('src.ml.gemma4_rules_model.download_weights_s3') as mock_download:
        model.init()
    mock_download.assert_called_once_with('gemma4', 'test-variant', str(tmp_path))
...
def test_init_loads_model_with_bfloat16_and_auto_device_map(tmp_path):
    (tmp_path / 'model.safetensors').touch()
    model = Gemma4RulesModel(model_variant='test-variant', model_weights_dir=str(tmp_path))
    with patch('src.ml.gemma4_rules_model.AutoModelForCausalLM') as mock_model_cls, \
         patch('src.ml.gemma4_rules_model.AutoProcessor'), \
...
def test_init_loads_processor_from_weights_dir(tmp_path):
    ...
    model = Gemma4RulesModel(model_variant='test-variant', model_weights_dir=str(tmp_path))
    with patch('src.ml.gemma4_rules_model.AutoModelForCausalLM'), \
         patch('src.ml.gemma4_rules_model.AutoProcessor') as mock_proc_cls, \
         patch('src.ml.gemma4_rules_model.download_weights_s3'):
        model.init()
    mock_proc_cls.from_pretrained.assert_called_once_with(str(tmp_path))
...
def test_init_assigns_loaded_model_and_processor_to_instance(tmp_path):
    ...
    with patch('src.ml.gemma4_rules_model.AutoModelForCausalLM') as mock_model_cls, \
         ...
         patch('src.ml.gemma4_rules_model.download_weights_s3'):
        model.init()
    assert model.model is mock_model_cls.from_pretrained.return_value
    assert model.processor is mock_proc_cls.from_pretrained.return_value
...
def test_init_runs_warmup_generate(tmp_path):
    ...
    with patch('src.ml.gemma4_rules_model.AutoModelForCausalLM') as mock_model_cls, \
         ...
         patch('src.ml.gemma4_rules_model.download_weights_s3'):
        model.init()
    mock_model_cls.from_pretrained.return_value.generate.assert_called_once()


# --- _build_rule_messages ---

def test_build_rule_messages_includes_two_system_messages(initialized_model, sample_rules):
    messages = initialized_model._build_rule_messages(sample_rules)
    system_messages = [m for m in messages if m['role'] == 'system']
    assert len(system_messages) == 2


def test_build_rule_messages_creates_one_user_message_per_rule(initialized_model, sample_rules):
    messages = initialized_model._build_rule_messages(sample_rules)
...
def test_build_rule_messages_formats_rule_as_question(initialized_model):
    rules = [RuleEvaluationInput(rule_public_id='1', rule_name='Test', rule='a cat is present')]
...
def test_build_rule_messages_includes_image_placeholder_in_each_user_message(initialized_model, sample_rules):
    messages = initialized_model._build_rule_messages(sample_rules)
    user_messages = [m for m in messages if m['role'] == 'user']
...
def test_build_rule_messages_with_no_rules_returns_only_system_messages(initialized_model):
...
def test_evaluate_rules_calls_apply_chat_template(initialized_model, sample_rules, sample_image):
...
def test_evaluate_rules_passes_one_user_message_per_rule_to_chat_template(
    ...
    initialized_model.evaluate_rules(rules=sample_rules, image=sample_image)
    messages = initialized_model.processor.apply_chat_template.call_args.args[0]
    user_messages = [m for m in messages if m['role'] == 'user']
    assert len(user_messages) == len(sample_rules)


def test_evaluate_rules_calls_model_generate(initialized_model, sample_rules, sample_image):
    initialized_model.evaluate_rules(rules=sample_rules, image=sample_image)
    initialized_model.model.generate.assert_called_once()


def test_evaluate_rules_returns_a_list(initialized_model, sample_rules, sample_image):
    result = initialized_model.evaluate_rules(rules=sample_rules, image=sample_image)
